###
###Aggregate Gov Communications to Tweet level
###5/08/2020
###Benjamin E. Bagozzi
###
...
twitter$date<-gsub("T.*","",twitter$created_at)
colnames(twitter)<-c("fulltime","text","date")
sentiment=sentiment_by(as.character(twitter$text))
twitter$sentiment<-sentiment$ave_sentiment
twitter<-twitter[,c("fulltime","date","sentiment")]
colnames(twitter)<-c("fulltime","date","twittersentiment")
...
matrixnew<-cbind(total$date,total$de.cases,total$de.deaths,total$md.cases,total$md.deaths,total$nj.cases,total$nj.deaths,total$usa.cases,total$usa.deaths,total$readability,total$sentiment,total$wordCount,total$NumPressRelease,total$NumNewsStories,matrixnew)
colnames(matrixnew)<-c("date","DEcases","DEdeaths","MDcases","MDdeaths","NJcases","NJdeaths","USAcases","USAdeaths","readability","sentiment","wordcount","pressrelasecount","newsstorycount",dtm.corpus[[6]]$Terms)
matrixnew<-as.data.frame(matrixnew)

twitter$date<-as.character(format(as.Date(twitter$date, "%Y-%m-%d")))
matrixnew$date<-as.character(format(as.Date(matrixnew$date, "%Y-%m-%d")))
#save dataset
write.csv(twitter,"Twitter.csv",row.names=FALSE)
write.csv(matrixnew,"Daily.csv",row.names=FALSE)
